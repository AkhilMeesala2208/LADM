//
//  LADMAmbientLightSensor.m
//  LADMCore
//
//  Created by Guilherme Rambo on 23/02/21.
//

#import <LADMCore/LADMCore.h>
...
@interface LADMAmbientLightSensor ()
...
@implementation LADMAmbientLightSensor
{
...
- (instancetype)init
{
    ...
    self.log = os_log_create(kLADMCoreSubsystemName, "AmbientLightSensor");
...
{
#if DEBUG
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"LADMFakeSystemUsesLegacySensor"]) return NULL;
#endif
...
- (void)_initializeLegacySensorObject
{
    ...
    if (!_legacySensorInitializedSuccessfully) {
        // kIOMainPortDefault was introduced in macOS 12 as a replacement for the
        // deprecated kIOMasterPortDefault. The #ifdef lets us compile cleanly on
        // both old and new SDKs — like a two-sided key.
#ifdef kIOMainPortDefault
        legacySensorObject = IOServiceGetMatchingService(kIOMainPortDefault, IOServiceMatching("AppleLMUController"));
#else
...
- (BOOL)isPresent
{
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"LADMFakeSensorSupported"]) return YES;
